      ...
      <tbody>
        {books.map((book) => (
          <tr key={book.id} className="hover:bg-gray-100">
            ...
            <td className="border p-2 w-70 break-words">{book.description}</td>
            <td className="border p-2">{book.rackNumber}</td>
            <td className="border p-2">{book.status}</td>
            <td className="border p-2 flex flex-col items-center gap-2">
              <button
                className="bg-black text-white p-1 rounded w-20"
                onClick={() => onEdit(book)}
              >
                Edit
              </button>
              <button
                className="bg-orange-700 text-white p-1 rounded w-20"
